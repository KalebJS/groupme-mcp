...
GROUPME_TOKEN=<token> uv run mcp dev main.py
...
The project is a thin MCP server over the GroupMe API v3, split across three files:

- **`client.py`** — `GroupMeClient`: async HTTP client wrapping the GroupMe REST API. Reads `GROUPME_TOKEN` from the environment at init. All methods are `async` and use `httpx.AsyncClient` per-request. The `_get`/`_post`/`_delete` helpers inject the token as a query param on every request.

- **`server.py`** — MCP tool definitions using `FastMCP`. Each `@mcp.tool()` function is a thin shim that delegates directly to `GroupMeClient`. Tools auto-generate `source_guid` (UUID) when the caller omits it, to prevent duplicate sends.

- **`main.py`** — Entry point; calls `mcp.run(transport="stdio")`.
...
1. Add the async method to `GroupMeClient` in `client.py`.
2. Add a corresponding `@mcp.tool()` function in `server.py` that calls the client method.
